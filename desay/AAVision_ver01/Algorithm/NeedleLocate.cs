...
namespace desay
{
    class NeedleLocate
    {
                                 ...
                                 Point3D<double> Needlepos, Point3D<double> CamPos)//图像处理代码
        {
            ...
            ho_Regions.Dispose();
            HOperatorSet.Regiongrowing(ho_ImageGauss, out ho_Regions, 4, 4, 2, areaMin);
            //选出圆度大于0.7的region
            ho_SelectedRegions.Dispose();
            HOperatorSet.SelectShape(ho_Regions, out ho_SelectedRegions, "circularity", "and", 0.7, 1);
            ...
            double offset_x = 1 * ((hv_target_circle_center_X[0] - (hv_Width[0] / 2)) * 0.0096);
            double offset_y = -1 * ((hv_target_circle_center_Y[0] - (hv_Height[0] / 2)) * 0.0096);
